    ...
    if (self.imageBtns_MARR.count) {
        ...
        switch ((int)longPG.state) {
            case UIGestureRecognizerStateBegan:
            {
                [self.imageBtns_MARR enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                    ...
                    if (CGRectContainsPoint(btn.frame, point)) {
                        ...
                        if ([self.delegate respondsToSelector:@selector(dragImageStart:)]) {
                            [self.delegate dragImageStart:idx];
                        }
                    ...
                    if (CGRectContainsPoint(btn.frame, point)) {
                        *stop = YES;
                        ...
                        if (self.from != self.to) {
                            ...
                            if ([self.delegate respondsToSelector:@selector(dragImageChange:)]) {
                                [self.delegate dragImageChange:idx];
                            }
                        }
                    }
                }];
            ...
            case UIGestureRecognizerStateEnded:
            {
                ...
                if ([self.delegate respondsToSelector:@selector(dragImageEnd:)])
                    [self.delegate dragImageEnd:self.from];
...
- (void)loadingImageView
{
    ...
    [self.images_MARR enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (self.bSelectLine == YES) {
            image_BTN.layer.borderColor = [UIColor redColor].CGColor;
            image_BTN.layer.borderWidth = 2.0f;

            UILabel *lblNumber = [[UILabel alloc] initWithFrame:CGRectMake(self.itemWidth-15, self.itemWidth-15, 15.0f, 15.0f)];
            lblNumber.backgroundColor = [UIColor redColor];
            lblNumber.font = [UIFont systemFontOfSize:12.0f];
            lblNumber.textColor = [UIColor whiteColor];
            lblNumber.textAlignment = NSTextAlignmentCenter;
            lblNumber.text = [NSString stringWithFormat:@"%lu", (unsigned long)idx + 1];

            lblNumber.tag = 9901;
            [image_BTN addSubview:lblNumber];
        }
    }];
