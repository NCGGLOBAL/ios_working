...
- (NSData*) encryptStringToData:(NSString*)plaintext withKey:(NSString*)key {
	...
	NSLog(@"plaintext[%@] key[%@]", plaintext, key);
#endif
...
#else
    return [[[NSString alloc] initWithData:[ciphertext AES256DecryptWithKey:key] encoding:NSUTF8StringEncoding] autorelease];
#endif
...
#if __has_feature(objc_arc)
    return [[NSString alloc] initWithData:[ciphertext AES128DecryptWithKey:key] encoding:NSUTF8StringEncoding];
#else
    return [[[NSString alloc] initWithData:[ciphertext AES128DecryptWithKey:key] encoding:NSUTF8StringEncoding] autorelease];
#endif
