...
//        _ivLodingTemp.image = [UIImage imageNamed:@"bg480h.png"];
    ...
    NSURL *sUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@/m", (NSString *)kWebSiteUrl]];
    ...
    if (nSnsType == 1) {
        ...
        [[KOSession sharedSession] openWithCompletionHandler:^(NSError *error) {
            if ([[KOSession sharedSession] isOpen]) {
                ...
                // failed
                [SysUtils showMessage:@"인증에 실패하였습니다."];
            }
...
- (void)transAferMsg:(NSDictionary *)aParams {
    if ([SysUtils isNull:aParams] == NO) {
        ...
        NSDictionary *dicResult = [NSDictionary dictionaryWithObjectsAndKeys:@"9999", @"resCode", @"통신이 실패하였습니다.", @"resCode", nil];
...
- (void)pictureResultData:(NSArray *)aData {
    if ([SysUtils isNull:_sTokenValue] == YES) {
        [SysUtils showMessage:@"토큰값이 존재하지 않습니다."];
        return;
    }
