...
- (void)viewDidLoad
{
    self.title = [NSString stringWithFormat:@"사진선택 %@/10", @(self.currentCount + self.choosedCount)];
...
- (void)doneAction:(id)sender
{
    if (self.choosedImages.count) {
        ...
        [self showTip:@"선택된 이미지가 없습니다." dismissDelay:1];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.asset.choosed) {
        ...
        if (self.currentCount + self.choosedCount >= 10) {
            [self showTip:@"10개만 등록 가능합니다." dismissDelay:1];
            return;
        }
