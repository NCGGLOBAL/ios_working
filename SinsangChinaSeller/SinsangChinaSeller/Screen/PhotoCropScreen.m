...
@interface PhotoCropScreen () {
    __weak IBOutlet UIView *_vTopMenu;
    __weak IBOutlet UIView *_vDownMenu;
...
@property (nonatomic, retain) UIRotateImageView *showImgView;
...
@implementation PhotoCropScreen
...
- (void)viewDidLoad
{
    ...
    [self.view bringSubviewToFront:_vDownMenu];
...
- (void)initView {
    ...
    [self.titleLabel setText:NSLocalizedString(@"str_edit_photo", comment: "")];
    [self.doneButton setTitle:NSLocalizedString(@"str_done", comment: "") forState:UIControlStateNormal];
    [self.rotateButton setTitle:NSLocalizedString(@"str_rotate", comment: "") forState:UIControlStateNormal];
    [self.cropButton setTitle:NSLocalizedString(@"str_cut", comment: "") forState:UIControlStateNormal];
    
    self.showImgView = [[UIRotateImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
...
- (IBAction)navRightButtonTouchUp:(id)sender {
    if (_bImageModGbn == YES && self.delegate && [self.delegate conformsToProtocol:@protocol(PhotoCropScreenDelegate)]) {
...
- (IBAction)rotateButtonClick:(id)sender {
    ...
    CIImage *imgToRotate = [CIImage imageWithCGImage:self.originalImage.CGImage];
    ...
    self.originalImage = [self fixOrientation:finalImage];
    
    [self.showImgView setImage:finalImage];
    
    [self adjustPossition];
}


- (IBAction)cropButtonClick:(id)sender {
    _bImageModGbn = YES;
    
    self.originalImage = [self getSubImage];
...
{
    CGAffineTransform saveState = self.showImgView.transform;
    
    self.showImgView.transform = CGAffineTransformIdentity;
    
    [self.showImgView setFrameToFitImage];
    
    //viewLayer.frame = imgvPhoto.frame;
    
    self.showImgView.transform = saveState;
}
