    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                if let imageUrl = info[UIImagePickerController.InfoKey.imageURL] as? URL {
                    ...
                    print("📷 ACT1037 - 선택된 이미지: \(imageName)")
                    var myDict = [String: Any]()
                    if let imageData = image.pngData() {
                        let base64String = imageData.base64EncodedString()
                        print("📷 ACT1037 - Base64 길이: \(base64String.count) characters")
                    ...
                    do {
                        ...
                        if let jsonString = String(data: jsonData, encoding: .utf8) {
                            print("📷 ACT1037 - 콜백 함수: \(self.callback)")
                            print("📷 ACT1037 - JSON 문자열 길이: \(jsonString.count) characters")
                            ...
                            webView.evaluateJavaScript(jsFunction, completionHandler: { (result, error) in
                                if let error = error {
                                    print("❌ ACT1037 - 콜백 전송 실패: \(error.localizedDescription)")
                                } else {
                                    print("✅ ACT1037 - 콜백 전송 성공: \(String(describing: result))")
                                }
                            })
                        } else {
                            print("❌ ACT1037 - JSON 문자열 변환 실패")
                        }
                    } catch {
                        print("❌ ACT1037 - JSON 변환 오류: \(error.localizedDescription)")
                    }
                } else {
                    print("❌ ACT1037 - 이미지 URL을 가져올 수 없습니다")
                }
            } else {
                print("❌ ACT1037 - 이미지를 가져올 수 없습니다")
            }
    }
