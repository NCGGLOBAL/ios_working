...
- (void)viewDidLoad {
    ...
    [self.titleLabel setText:NSLocalizedString(@"str_reg_product", comment: "")];
    
    [self.doneButton setTitle:NSLocalizedString(@"str_done", comment: "") forState:UIControlStateNormal];
    
    [self.directLabel setText:NSLocalizedString(@"str_direct_capture", comment: "")];
    
    [self.selectLabel setText:NSLocalizedString(@"str_select_photo", comment: "")];
}
...
- (IBAction)cameraButtonClick:(id)sender {
    if ([_arrPictureImage count] >= 10) {
        [SysUtils showMessage:NSLocalizedString(@"str_reg_limit", comment: "")];
        return;
    }
